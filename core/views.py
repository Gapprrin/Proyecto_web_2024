...
def addToShopKart(request, id):
    accesorio = Accesorios_desc.objects.get(id = id)
    ...
    for item in carrito:
        ...
        carrito.append({"id" : id, "nombre" : accesorio.nombre_accesorio, "imagen" : accesorio.imagen_accesorio,
                        "cantidad" : 1, "precio" : accesorio.precio_accesorio, "subtotal" : accesorio.precio_accesorio})
        monto[0]["subtotal"] += accesorio.precio_accesorio
        monto[0]["total"] += accesorio.precio_accesorio
    request.session["carrito"] = carrito
    request.session["monto"] = monto
    return redirect(reverse('home') + '#contenedor_descuentos_acc')
